...
Run:
`bazel build tensorflow/lite/micro/examples/meta:train`
`bazel-bin/tensorflow/lite/micro/examples/meta/train --save_tf_model --save_dir=/tmp/model_created/`
"""
import math
import os
...
flags.DEFINE_string("save_dir", "/tmp/meta_models",
...
def convert_tflite_model(model):
  """Convert the save TF model to tflite model, then save it as .tflite flatbuffer format
    Args:
        model (tf.keras.Model): the trained meta Model
    Returns:
...
def train_model(epochs, x_values, y_values):
  """Train keras meta model
    ...
    Returns:
        tf.keras.Model: A trained keras meta model
  """
  model = create_model()
  model.fit(x_values,
            y_values,
