...
from tflite_micro.tensorflow.lite.micro.examples.meta import evaluate

PREFIX_PATH = resource_loader.get_path_to_datafile('')


class MetaFloatModelTest(test_util.TensorFlowTestCase):
  model_path = os.path.join(PREFIX_PATH, 'models/meta_float.tflite')
  input_shape = (1, 1)
  output_shape = (1, 1)
...
class MetaQuantModelTest(test_util.TensorFlowTestCase):
  model_path = os.path.join(PREFIX_PATH, 'models/meta_int8.tflite')
  input_shape = (1, 1)
  output_shape = (1, 1)
  tflm_interpreter = runtime.Interpreter.from_file(model_path)
