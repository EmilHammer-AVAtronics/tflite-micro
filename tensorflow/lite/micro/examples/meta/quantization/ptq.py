...
`bazel build tensorflow/lite/micro/examples/meta/quantization:train`

The following command first creates the trained TF float model that we will quantize later
`bazel-bin/tensorflow/lite/micro/examples/meta/train --save_tf_model --save_dir=/tmp/float_model/`

Build the ptq.py script
`bazel build tensorflow/lite/micro/examples/meta/quantization:ptq`
...
The quant model (named meta_int8.tflite) will be created inside the target_dir.
`bazel-bin/tensorflow/lite/micro/examples/meta/quantization/ptq  --source_model_dir=/tmp/float_model --target_dir=/tmp/quant_model/`
"""
import math
import os
...
def convert_quantized_tflite_model(source_model_dir, x_values):
  """Convert the save TF model to tflite model, then save it as .tflite
    ...
    Args:
        source_model_dir (tf.keras.Model): the trained meta flaot Model dir
