...
// #include
// "tensorflow/lite/micro/examples/avatronics/models/DPCRN_m71_quant_full_int_InOuts_float32_model_data.h"

/*======================== Included from GitHub folder
 * ==========================*/
// #include
// "../../shared_ubuntu/AVAHEEAR/AVA_DPCRN/pretrained_weights/DPCRN_S/airoha_models/DPCRN_m62_quant_full_int_InOuts_float32_7_6_2024_model_data.h"
// #include
// "../../shared_ubuntu/AVAHEEAR/AVA_DPCRN/pretrained_weights/DPCRN_S/airoha_models/DPCRN_m71_quant_full_int_InOuts_float32_13_6_2024_model_data.h"
// #include
// "../../shared_ubuntu/AVAHEEAR/AVA_DPCRN/pretrained_weights/DPCRN_S/airoha_models/DPCRN_m71_quant_full_int_InOuts_float32_model_data.h"
...
int8_t nn_setup() {
  ...
  model = ::tflite::GetModel(
      g_DPCRN_m71_quant_full_int_InOuts_float32_13_6_2024_model_data);
...
template <typename T>
int8_t nn_inference(int8_t* numElementsInputTensors, T** inputTensors,
                    const int32_t* inputTensorValues,
                    int8_t* numElementsOutputTensors, T** outputTensors,
                    const int32_t* outputTensorValues) {
...
int inputs_size() { return global_interpreter->inputs_size(); }

int outputs_size() { return global_interpreter->outputs_size(); }

int initialization_status() {
  return global_interpreter->initialization_status();
}

int tflm_return_value_test(int8_t* numElementsInputTensors,
                           double** inputTensors,
                           int8_t* numElementsOutputTensors,
                           double** outputTensors) {
  /*Input*/
...
int avatronics_test() {
  ...
  int8_t numElementsInputTensors =
      sizeof(inputTensors) / sizeof(inputTensors[0]);
...
#if MODEL_DATATYPE_INT32
  int32_t* outputTensors[] = {
      outputTensor_0, outputTensor_1, outputTensor_2,
      // outputTensor_3
  };
#elif MODEL_DATATYPE_DOUBLE || FULL_INT_INOUTS_FLOAT32
  double* outputTensors[] = {
  ...
  int8_t numElementsOutputTensors =
      sizeof(outputTensors) / sizeof(outputTensors[0]);
  for (int i = 0; i < numElementsOutputTensors; i++) {
  ...
  if (nn_inference(&numElementsInputTensors, inputTensors, inputTensorShapes,
                   &numElementsOutputTensors, outputTensors,
                   outputTensorShapes)) {
